...
trait AbstractDeclarationModel extends DomainElementModel with KeyField with NameFieldSchema with DescriptionField {
  ...
  val DataNode = Field(DataNodeModel, Document + "dataNode", ModelDoc(ModelVocabularies.AmlDoc, "data node", "Associated dynamic structure for the declaration"))

  val Variables = Field(Array(Str), Document + "variable", ModelDoc(ModelVocabularies.AmlDoc, "variable", "Variables to be replaced in the graph template introduced by an AbstractDeclaration"))
...
object AbstractDeclarationModel extends AbstractDeclarationModel {
  ...
  override  val doc: ModelDoc = ModelDoc(ModelVocabularies.AmlDoc, "Abstract Declaration", "Graph template that can be used to declare a re-usable graph structure that can be applied to different domain elements\nin order to re-use common semantics. Similar to a Lisp macro or a C++ template.\nIt can be extended by any domain element adding bindings for the variables in the declaration.")
}
