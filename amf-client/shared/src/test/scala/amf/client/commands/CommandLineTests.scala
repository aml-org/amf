...
class CommandLineTests extends AsyncFunSuite with PlatformSecrets {
  ...
  test("Parse command") {
    val args = Array("parse", "-in", "RAML 1.0", "-mime-in", "application/yaml", "file://amf-client/shared/src/test/resources/upanddown/complete-with-operations.raml")
  ...
  test("Translate command") {
    val args = Array("translate", "-in", "RAML 1.0", "-mime-in", "application/yaml", "-out", "OAS 2.0", "-mime-out", "application/json", "file://amf-client/shared/src/test/resources/upanddown/complete-with-operations.raml")
    val cfg = CmdLineParser.parse(args)
    assert(cfg.isDefined)
    val stdout = new TestWriter()
    val stderr = new TestWriter()
  ...
  test("Validation command") {
    val args = Array("validate", "-in", "RAML 1.0", "-mime-in", "application/yaml", "-p", "RAML", "file://amf-client/shared/src/test/resources/validations/data/error1.raml")
    val cfg = CmdLineParser.parse(args)
    assert(cfg.isDefined)
  ...
  test("Custom validation command") {
    val args = Array("validate",
      ...
      "-cp", "file://amf-client/shared/src/test/resources/validations/data/custom_function_validation_error.raml",
  ...
  test("Dialects parsing command") {
    val args = Array("parse",
      "-in", "RAML Vocabulary",
      "-mime-in", "application/yaml",
      "-ds", "file://amf-client/shared/src/test/resources/vocabularies/k8/dialects/pod.raml",
      "file://amf-client/shared/src/test/resources/vocabularies/k8/examples/pod.raml")
    val cfg = CmdLineParser.parse(args)
    assert(cfg.isDefined)
