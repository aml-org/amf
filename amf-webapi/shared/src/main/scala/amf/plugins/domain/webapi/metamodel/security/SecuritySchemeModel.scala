...
object SecuritySchemeModel extends DomainElementModel with KeyField with DescriptionField with DisplayNameField {
  ...
  val Headers = Field(Array(ParameterModel), Http + "header", ModelDoc(ModelVocabularies.Http, "header", "Security scheme specific headers"))

  val QueryParameters = Field(Array(ParameterModel), Http + "parameter", ModelDoc(ModelVocabularies.Http, "parameter", "Security scheme specific parameters"))

  val Responses = Field(Array(ResponseModel), Hydra + "response", ModelDoc(ExternalModelVocabularies.Hydra, "response", "Response associated to this security scheme"))

  val Settings = Field(SettingsModel, Security + "settings", ModelDoc(ModelVocabularies.Security, "settings", "Security scheme settings"))

  val QueryString = Field(ShapeModel, Http + "queryString", ModelDoc(ModelVocabularies.Http, "query string", "Query string associated to this security scheme"))
