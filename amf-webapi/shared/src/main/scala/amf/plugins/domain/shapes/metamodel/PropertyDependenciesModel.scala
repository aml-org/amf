...
object PropertyDependenciesModel extends DomainElementModel {
  ...
  val PropertyAppliedShape = Field(
    ShapeModel,
    Shapes + "propertyAppliedShape",
    ModelDoc(ModelVocabularies.Shapes, "propertyAppliedShape", "Target applied shape in the dependency"))

  override def fields: List[Field] =
    List(PropertySource, PropertyTarget, PropertyAppliedShape) ++ DomainElementModel.fields
