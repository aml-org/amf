...
class ReferencesMakerTest extends AsyncFunSuite with CompilerTestBuilder with AmfObjectTestMatcher {
  ...
  test("Data type fragment test raml") {
    val rootDocument = "file://parser-client/shared/src/test/resources/references/data-type-fragment.reference.raml"
    assertFixture(rootDocument, RamlYamlHint)
  }

  test("Data type fragment test oas") {
    val rootDocument = "file://parser-client/shared/src/test/resources/references/data-type-fragment.json"
  ...
  case class UnitsCreator(spec: Vendor) {
    ...
    private val dataTypeFragment: Fragment = {
      DataTypeFragment()
        .withLocation("file://parser-client/shared/src/test/resources/references/fragments/" + fragmentFile)
        .withId("file://parser-client/shared/src/test/resources/references/fragments/" + fragmentFile)
        .withEncodes(person)
    }
    ...
    val usesDataType: Document = {
      ...
      Document()
        .withId("/Users/hernan.najles/mulesoft/amf/parser-client/shared/src/test/resources/references/" + file)
        .withLocation("/Users/hernan.najles/mulesoft/amf/parser-client/shared/src/test/resources/references/" + file)
        .withEncodes(
          WebApi()
            .withId("parser-client/shared/src/test/resources/references/" + file + "#/web-api")
            .withName("API")
            .withVersion("1.0"))
