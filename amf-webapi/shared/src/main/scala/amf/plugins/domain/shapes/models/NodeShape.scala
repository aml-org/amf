    ...
    dependency.fields.entry(PropertyDependenciesModel.PropertySource).foreach {
      case entry @ FieldEntry(_, value) =>
        ...
        oldIdToPropertyMap.get(oldPropertyId).foreach { property =>
          dependency.set(PropertyDependenciesModel.PropertySource, AmfScalar(property.id, entry.element.annotations), value.annotations)
        }
    }
    dependency.fields.entry(PropertyDependenciesModel.PropertyTarget).foreach {
      case entry @ FieldEntry(_, value) =>
        val dependents = entry.array.scalars
        val updatedDependents = dependents.flatMap { field => oldIdToPropertyMap.get(field.value.toString) }.map(p => AmfScalar(p.id))
        dependency.set(PropertyDependenciesModel.PropertyTarget, AmfArray(updatedDependents, entry.element.annotations), value.annotations)
    }
  }
