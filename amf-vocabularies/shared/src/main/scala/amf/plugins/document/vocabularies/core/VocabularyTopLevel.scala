...
object RAML_1_0_VocabularyTopLevel {
  case class VocabularyObject(entity: DomainEntity = DomainEntity(Vocabulary),
    ...
    def withUses(value: NameSpaceImportObject): VocabularyObject = {
      entity.add(Vocabulary.uses.field(), value.entity); this
    }
  ...
  case class NameSpaceImportObject(entity: DomainEntity = DomainEntity(NameSpaceImport),
    ...
    def withName(value: String): NameSpaceImportObject = {
      entity.set(NameSpaceImport.name.field(), AmfScalar(value)); this
    }
    def uri(): Option[String] = entity.string(NameSpaceImport.uri)
    def withUri(value: String): NameSpaceImportObject = {
      entity.set(NameSpaceImport.uri.field(), AmfScalar(value)); this
    }
  }
