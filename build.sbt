...
lazy val brokerGroup = crossProject(JSPlatform, JVMPlatform)
  .settings(
    Seq(
      name := "amf-broker-group"
    )
  )
  .in(file("./amf-broker-group"))
  .settings(commonSettings)
  .dependsOn(shapes)
  .jvmSettings(
    libraryDependencies += "org.scala-js" %% "scalajs-stubs" % "1.1.0" % "provided",
    Compile / packageDoc / artifactPath   := baseDirectory.value / "target" / "artifact" / "amf-broker-group-javadoc.jar",
    Compile / packageBin / mappings += file("amf-apicontract.versions") -> "amf-apicontract.versions"
  )
  .jsSettings(
    scalaJSLinkerConfig ~= { _.withModuleKind(ModuleKind.CommonJSModule) },
    Compile / fullOptJS / artifactPath := baseDirectory.value / "target" / "artifact" / "amf-broker-group.js",
    npmDependencies ++= npmDeps
  )
  .settings(AutomaticModuleName.settings("amf.broker-group"))

lazy val brokerGroupJVM =
  brokerGroup.jvm
    .in(file("./amf-broker-group/jvm"))
    .disablePlugins(SonarPlugin)

lazy val brokerGroupJS =
  brokerGroup.js
    .in(file("./amf-broker-group/js"))
    .disablePlugins(SonarPlugin, ScoverageSbtPlugin)
...
lazy val agentMetadata = crossProject(JSPlatform, JVMPlatform)
  ...
  .jsSettings(
    scalaJSLinkerConfig ~= { _.withModuleKind(ModuleKind.CommonJSModule) },
    Compile / fullOptJS / artifactPath := baseDirectory.value / "target" / "artifact" / "amf-agent-metadata.js",
    npmDependencies ++= npmDeps
  )
  .settings(AutomaticModuleName.settings("amf.agent-metadata"))

lazy val agentMetadataJVM =
  agentMetadata.jvm
    .in(file("./amf-agent-metadata/jvm"))
    .disablePlugins(SonarPlugin)

lazy val agentMetadataJS =
  agentMetadata.js
    .in(file("./amf-agent-metadata/js"))
    .disablePlugins(SonarPlugin, ScoverageSbtPlugin)
...
lazy val llmMetadata = crossProject(JSPlatform, JVMPlatform)
  ...
  .jsSettings(
    scalaJSLinkerConfig ~= { _.withModuleKind(ModuleKind.CommonJSModule) },
    Compile / fullOptJS / artifactPath := baseDirectory.value / "target" / "artifact" / "amf-llm-metadata.js",
    npmDependencies ++= npmDeps
  )
  .settings(AutomaticModuleName.settings("amf.llm-metadata"))

lazy val llmMetadataJVM =
  llmMetadata.jvm
    .in(file("./amf-llm-metadata/jvm"))
    .disablePlugins(SonarPlugin)

lazy val llmMetadataJS =
  llmMetadata.js
...
lazy val cli = crossProject(JSPlatform, JVMPlatform)
  ...
  .dependsOn(grpc, graphql, mcp, brokerGroup, agentCard, otherCard, agentMetadata, llmMetadata)
  .in(file("./amf-cli"))
  .settings(commonSettings)
  .settings(
