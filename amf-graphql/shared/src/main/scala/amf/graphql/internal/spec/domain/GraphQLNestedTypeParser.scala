...
import amf.graphqlfederation.internal.spec.domain.{KeyParser, FederationMetadataParser}
  ...
  def parse(): NodeShape = {
    ...
    if (isInterface) {
      ...
      inFederation { implicit fCtx =>
        FederationMetadataParser(objTypeNode, obj, Seq(INTERFACE_DIRECTIVE, INTERFACE_FEDERATION_DIRECTIVE))
          .parse()
    ...
    inFederation { implicit fCtx =>
      FederationMetadataParser(objTypeNode, obj, Seq(OBJECT_DIRECTIVE, OBJECT_FEDERATION_DIRECTIVE)).parse()
