...
import amf.core.annotations.{AutoGeneratedName, ExplicitField, SynthesizedField}
      ...
      case YType.Null =>
        Raml10TypeParser(entry,
          ...
          .foreach { schema =>
            ...
            payload.withSchema(tracking(schema.add(AutoGeneratedName()), payload.id))
          }
      case _ =>
        Raml10TypeParser(entry,
                         shape => shape.withName("schema").adopted(payload.id),
                         ...
                         AnyDefaultType)
          .parse()
          .foreach(s => payload.withSchema(tracking(s.add(AutoGeneratedName()), payload.id)))
