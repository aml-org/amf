...
case class WebApi(fields: Fields, annotations: Annotations) extends DomainElement {
  ...
  def baseUriParameters: Seq[Parameter] = fields(BaseUriParameters)def security: Seq[ParametrizedSecurityScheme] = fields(Security)
  ...
  def withBaseUriParameters(parameters: Seq[Parameter]): this.type = setArray(BaseUriParameters, parameters)def withSecurity(security: Seq[ParametrizedSecurityScheme]): this.type = setArray(Security, security)
