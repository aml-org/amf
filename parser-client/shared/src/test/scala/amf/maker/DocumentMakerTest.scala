...
class DocumentMakerTest extends WebApiMakerTest {
  ...
  test("Raml declared types ") {
    val doc = documentWithTypes(Raml)
      .withLocation("file://parser-client/shared/src/test/resources/maker/declared-types.raml")
    assertFixture(doc, "declared-types.raml", RamlYamlHint)
  }

  test("Oas declared types ") {
    val doc = documentWithTypes(Oas)
      .withLocation("file://parser-client/shared/src/test/resources/maker/declared-types.json")
    assertFixture(doc, "declared-types.json", OasJsonHint)
  }

  test("Raml inherits declared types ") {
    val doc = documentWithInheritsTypes(Raml)
      .withLocation("file://parser-client/shared/src/test/resources/maker/inherits-declared-types.raml")
  ...
  test("Oas inherits declared types ") {
    val doc = documentWithInheritsTypes(Oas)
      .withLocation("file://parser-client/shared/src/test/resources/maker/inherits-declared-types.json")
  ...
  private def documentWithInheritsTypes(vendor: Vendor) = {
    ...
    val id = vendor match {
      case Oas =>
        "file://parser-client/shared/src/test/resources/maker/inherits-declared-types.json#/declarations/types/Human"
      case _ =>
        "file://parser-client/shared/src/test/resources/maker/inherits-declared-types.raml#/declarations/types/Human"
    }
