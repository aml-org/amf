...
class ResponseExamplesResolutionStage() extends TransformationStep() {
  ...
  def resolveApi(webApi: Api)(implicit errorHandler: AMFErrorHandler): Api = {
    ...
    allResponses.zipWithIndex.foreach { case (response, index) =>
      ...
      examplesByMediaType.foreach { case (mediaType, example) =>
  ...
  private def violationForUnmappedExample(example: Example, mediaType: String, payloads: Seq[Payload])(implicit
      errorHandler: AMFErrorHandler
  ): Unit = {
    if (payloads.isEmpty)
