...
class E2ERenderConfigurationSetupTest extends ConfigurationSetupTest with FileAssertionTest {
  ...
  val renderCases: Seq[Any] = Seq(
    ...
    generateRenderCases(
      raml08Config,
      webApiModel,
      Seq((`application/ld+json`, "webapi-model.jsonld"), (`application/yaml`, "raml08-api.raml"))
    ),
    generateRenderCases(
      raml10Config,
      webApiModel,
      Seq((`application/ld+json`, "webapi-model.jsonld"), (`application/yaml`, "raml10-api.raml"))
    ),
    ...
    generateRenderCases(
      ...
      webApiModel,
      Seq(
        (`application/ld+json`, "webapi-model.jsonld"),
        (`application/yaml`, "oas20-api.yaml"),
        (`application/json`, "oas20-api.json")
      )
    ),
    generateRenderCases(
      oas30Config,
      ...
      Seq(
        (`application/ld+json`, "webapi-model.jsonld"),
        (`application/yaml`, "oas30-api.yaml"),
        (`application/json`, "oas30-api.json")
      )
    )
  ).flatten
  ...
  renderCases.foreach {
    case e: ExpectedRenderCase =>
      ...
      test(
        s"Test - config ${configNames(e.config)} with ${e.mediaTypeAndGolden._1} and ${asyncOrWebApi} renders as expected"
      ) {
    ...
    case e: ErrorRenderCase =>
      ...
      val asyncOrWebApi = if (e.model.asInstanceOf[Document].encodes.isInstanceOf[WebApi]) "WebApi" else "AsyncApi"
      test(
        s"Test - config ${configNames(e.config)} with ${e.mediaType} and ${asyncOrWebApi} throws exception on render"
      ) {
  ...
  def generateRenderCases(
      config: AMFConfiguration,
      model: BaseUnit,
      mediaTypeAndGoldens: Seq[(String, String)]
  ): Seq[Any] = {
