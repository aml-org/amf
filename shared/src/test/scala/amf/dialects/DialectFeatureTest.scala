...
class DialectFeatureTest extends AsyncFunSuite with PlatformSecrets  {
  ...
  test("Default values") {
    ...
    val expected = platform.resolve(basePath + "validation_profile_with_default_example.json", None).map(_.stream.toString)
  ...
  test("Uses (in dialect definition)") {
    ...
    val expected = platform.resolve(basePath + "validation_profile_with_uses_example.json", None).map(_.stream.toString)
    val actual = validation
      .flatMap(unit =>
        AMFCompiler(basePath + "validation_profile_example.raml", platform, RamlYamlHint, None, None, platform.dialectsRegistry)
          .build()
  ...
  test("Uses (in dialect)") {
    ...
    val expected = platform.resolve(basePath + "validation_profile_example_uses_gold.raml", None).map(_.stream.toString)
    val actual = validation
      .flatMap(unit => {
  ...
  test("Uses (in dialect (real library))") {
    val validation =platform.dialectsRegistry.registerDialect(basePath + "validation_dialect_unions.raml")
    val expected = platform.resolve(basePath + "validation_profile_example_uses_gold2.raml", None).map(_.stream.toString)
    val actual = validation
      .flatMap(unit => {
        val dl=new DialectRegistry();
        dl.add(unit)
