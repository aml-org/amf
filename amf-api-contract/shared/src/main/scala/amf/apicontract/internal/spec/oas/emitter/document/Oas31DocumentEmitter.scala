...
case class Oas31DocumentEmitter(document: BaseUnit)(implicit override val specCtx: OasSpecEmitterContext)
    extends OasDocumentEmitter(document) {

  override protected def versionEntry(b: YDocument.EntryBuilder): Unit =
    b.openapi = YNode(YScalar("3.1.0"), YType.Str) // this should not be necessary but for use the same logic
