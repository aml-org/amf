...
public class YAMLUtil {
  ...
  public static YAMLKeyValue createI18nRecord(final YAMLFile file, final String[] key, final String text) {
    ...
    if (rootMapping == null) {
      final YAMLFile yamlFile = YamlElementGenerator.apply(file.getProject()).createDummyYamlWithText(key[0] + ":");
    ...
    for (i = 0; i < keyLength; i++) {
      ...
      final YAMLFile fileWithKey = YamlElementGenerator.apply(file.getProject()).createDummyYamlWithText(builder.toString());
  ...
  public static PsiElement rename(final YAMLKeyValue element, final String newName) {
    ...
    final YAMLKeyValue topKeyValue = YamlElementGenerator.apply(element.getProject()).createYamlKeyValue(newName, "Foo");
