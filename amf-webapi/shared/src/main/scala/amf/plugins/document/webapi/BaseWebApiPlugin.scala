...
trait BaseWebApiPlugin extends AMFDocumentPlugin with AMFValidationPlugin with WebApiValidations with PlatformSecrets {
  ...
  override def pipelines: Map[String, ResolutionPipeline] = Map(
    ResolutionPipeline.DEFAULT_PIPELINE -> UnifiedDefaultPipeline(),
    ResolutionPipeline.EDITING_PIPELINE -> UnifiedEditingPipeline(),
    ResolutionPipeline.CACHE_PIPELINE   -> new UnifiedCachePipeline()
  )
