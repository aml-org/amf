...
trait WrapperTests extends MultiJsonldAsyncFunSuite with Matchers with NativeOps with FileAssertionTest {
  ...
  test("Render / parse test RAML 0.8") {
    ...
    for {
      ...
      output = client.render(unit.baseUnit, `application/yaml`)
  ...
  test("Render / parse test OAS 2.0") {
    ...
    for {
      unit <- client.parse(zencoder).asFuture
      output = oas20Conf.baseUnitClient().render(unit.baseUnit, `application/json`)
  ...
  test("Render / parse test AMF") {
    ...
    for {
      unit <- client.parse(zencoder).asFuture
      output = client.render(unit.baseUnit, `application/ld+json`)
      result <- AMFParser.parseContent(output, configuration).asFuture
    } yield {
      assertBaseUnit(result.baseUnit, "http://a.ml/amf/default_document")
    }
  ...
  test("Resolution test") {
    ...
    for {
      unit <- client.parse(zencoder).asFuture
      resolved = client.transform(unit.baseUnit)
  ...
  test("Raml to oas security scheme after resolution") {
    ...
    for {
      ...
      resolved = transformClient.transform(unit.baseUnit).baseUnit
      output   = renderClient.render(resolved)
    } yield {
...
//      .withProperties(Seq("http://a.ml/vocabularies/doc#test").asClient)
  ...
  test("Generate to doc builder") {
    ...
    for {
      ...
      result = client.renderGraphToBuilder(unit.baseUnit, builder)
    } yield {
  ...
  test("Generate unit with source maps") {
    ...
    for {
      ...
      jsonld = client.render(unit.baseUnit, Mimes.`application/ld+json`)
    } yield {
  ...
  test("Test query parameters order") {
    ...
    for {
      ...
      v = client.transform(unit.baseUnit)
    } yield {
      val seq = v.baseUnit
        .asInstanceOf[Document]
        .encodes
  ...
  test("Test custom domain property id after parse") {
    ...
    for {
      ...
      s       = client.render(doc, `application/yaml`)
      loaders = List(StringResourceLoader(url, s, Some(Mimes.`application/yaml`)))
      withLoader = client
        .getConfiguration()
  ...
  test("Test JSON Schema emission without documentation") {
    ...
    for {
      unit <- client.parseContent(api).asFuture
      resolved = client.transform(unit.baseUnit, PipelineId.Editing)
      report <- client.validate(resolved.baseUnit).asFuture
    } yield {
      val expectedSchema =
        """{
  ...
  test("Resolve KG Service API") {
    ...
    for {
      ...
      resolved = client.transform(parsed.baseUnit, PipelineId.Editing)
    } yield {
  ...
  test("AutogeneratedName annotation should exists for anonymous union") {
    val client = RAMLConfiguration.RAML10().baseUnitClient()
    val api =
      """
        |#%RAML 1.0
    ...
    for {
      ...
      assert(parsingResult.conforms)
      assert(resolutionResult.conforms)
      val schema = resolutionResult.baseUnit
        .asInstanceOf[Document]
        .encodes
        .asInstanceOf[WebApi]
        .endPoints
        ...
        .head
        .schema
      assert(schema.annotations()._internal.find(classOf[AutoGeneratedName]).nonEmpty)
    }
  }

  // This is to emulate what a FS could resolve with a relative path starting with ../
  test("Test API with RT using a relative path starting with ../") {

    val rootPath = "../folder/main.raml"
    ...
    val main =
      s"""#%RAML 1.0
         ...
         |
         |resourceTypes:
         |    myResource: !include resource.raml
         |
         |/top:
    ...
    case class TestResourceLoader() extends ClientResourceLoader {
      ...
      private val index = Map[String, String](rootPath -> main, "../folder/resource.raml" -> rt)

      override def fetch(resource: String): ClientFuture[Content] =
        Future.successful(new Content(index(resource), resource)).asClient
    ...
    val config = RAMLConfiguration.RAML10().withResourceLoader(TestResourceLoader().asInstanceOf[ClientLoader])
    for {
      parsingResult    <- config.baseUnitClient().parse(rootPath).asFuture
      validationReport <- config.baseUnitClient().validate(parsingResult.baseUnit).asFuture
    } yield {
      assert(parsingResult.conforms)
      assert(validationReport.conforms)
      parsingResult.baseUnit shouldBe a[Document]
      val declarations = parsingResult.baseUnit.asInstanceOf[Document].declares.asSeq
      declarations should have size 1
    }
