...
class AMFShapeValidations(shape: Shape) {
  ...
  protected def scalarConstraints(context: String, scalar: ScalarShape): List[ValidationSpecification] = {
    if (scalar.format.option().isDefined && scalar.format.value() == "RFC2616" && scalar.dataType.value().endsWith("dateTime")) {
      ...
      var validation = new ValidationSpecification(
        ...
        propertyConstraints = Seq(
          PropertyConstraint(
            ...
            message = Some(msg),
            pattern = Some("((Mon|Tue|Wed|Thu|Fri|Sat|Sun), [0-9]{2} (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) [0-9]{4} [0-9]{2}:[0-9]{2}:[0-9]{2} GMT)")
          ))
      )
      List(validation)
    } else {
