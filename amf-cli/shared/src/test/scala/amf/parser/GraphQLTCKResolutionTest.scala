...
class GraphQLTCKResolutionTest extends GraphQLFunSuiteCycleTests {
  ...
  /** We should not resolve inheritance in GraphQL, only validate it We should detect recursions
    *
    * Calling ShapeNormalization will resolve inheritance and detect recursions. We only want the later. Skipping
    * recursive tests
    */
  ...
  fs.syncFile(s"$basePath").list.foreach { api =>
    if (api.endsWith(".graphql") && !api.endsWith(".dumped.graphql") && !ignored.contains(api)) {
      test(s"GraphQL TCK > Apis > Valid > $api: resolved dumped JSON matches golden") {
        cycle(
          api,
          api.replace(".graphql", ".resolved.jsonld"),
          GraphQLHint,
          AmfJsonHint,
          transformWith = Some(Spec.GRAPHQL)
        )
      }
    }
  }
