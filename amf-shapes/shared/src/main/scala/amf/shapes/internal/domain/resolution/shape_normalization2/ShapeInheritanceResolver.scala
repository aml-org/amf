...
private[resolution] object ShapeInheritanceResolver {
  def apply(s: Shape, context: NormalizationContext2): Shape = ShapeInheritanceResolver()(context).normalize(s)
}

sealed case class ShapeInheritanceResolver()(implicit val context: NormalizationContext2) extends ShapeNormalizer2 {
  ...
  override protected def normalizeAction(shape: Shape): Shape = {
    shape match {
      ...
      case s if hasSuperTypes(s) =>
        ...
        resolvedShape
      case any: AnyShape => AnyShapeAdjuster2(any) // Analyze if it's possible to remove this case. Affects MinShape?
      case _             => shape
    }
  }
