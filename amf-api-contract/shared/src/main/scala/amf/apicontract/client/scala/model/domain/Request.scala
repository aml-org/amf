...
class Request(override val fields: Fields, override val annotations: Annotations)
  ...
  //override def withQueryParameters(parameters: Seq[Parameter]): this.type        = super.withQueryParameters(parameters)
  ...
  override protected def buildQueryParameter: Parameter = Parameter()

  override def withQueryParameter(name: String): Parameter = {
    super.withQueryParameter(name).asInstanceOf[Parameter]
  }
