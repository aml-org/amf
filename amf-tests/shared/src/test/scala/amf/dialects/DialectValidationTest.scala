...
class DialectValidationTest extends AsyncFunSuite with PlatformSecrets {
  ...
  test("Basic Validation Test") {
    ...
    val cm = dl.flatMap(
      d =>
        AMFCompiler("file://amf-tests/shared/src/test/resources/vocabularies/muleconfig.raml",
                    platform,
                    RamlYamlHint,
                    Validation(platform),
                    None,
  ...
  test("another validation test") {
    ...
    val cm = dl.flatMap(
      d =>
        AMFCompiler("file://amf-tests/shared/src/test/resources/vocabularies/muleconfig2.raml",
                    platform,
                    RamlYamlHint,
                    Validation(platform),
                    None,
  ...
  test("missing required property") {
    ...
    val cm = dl.flatMap(
      d =>
        AMFCompiler("file://amf-tests/shared/src/test/resources/vocabularies/muleconfig3.raml",
                    platform,
                    RamlYamlHint,
                    Validation(platform),
                    None,
  ...
  test("Empty Vocabulary") {
    val validation = Validation(platform)
    AMFCompiler(
      "file://amf-tests/shared/src/test/resources/vocabularies/empty.raml",
      platform,
      RamlYamlHint,
      validation,
      None,
  ...
  test("Vocabulary can be validated with closed nodes") {
    ...
    AMFCompiler(
      "file://amf-tests/shared/src/test/resources/vocabularies/vocabulary_closed_shape_invalid.raml",
      platform,
      RamlYamlHint,
      validation,
      None,
  ...
  test("Custom dialect can be validated") {
    val validation               = Validation(platform)
    var dialect: Option[Dialect] = None
    val dialectFile              = "file://amf-tests/shared/src/test/resources/dialects/mule_configuration/configuration_dialect.raml"
    val dialectExampleFile       = "file://amf-tests/shared/src/test/resources/dialects/mule_configuration/example.raml"
  ...
  test("Vocabulary can be validated with closed nodes (k8)") {
    ...
    AMFCompiler(
      "file://amf-tests/shared/src/test/resources/vocabularies/k8/vocabulary/core.raml",
      platform,
      RamlYamlHint,
      validation,
      None,
  ...
  test("Vocabulary can be validated with references") {
    ...
    AMFCompiler(
      "file://amf-tests/shared/src/test/resources/vocabularies/broken_core.raml",
      platform,
      RamlYamlHint,
      validation,
      None,
  ...
  test("Dialect can be validated (k8)") {
    ...
    var dialect: Option[Dialect] = None
    validator.loadValidationDialect().flatMap { parsedDialect =>
      AMFCompiler("file://amf-tests/shared/src/test/resources/vocabularies/k8/dialects/pod.raml", platform, RamlYamlHint, validator, None, None, platform.dialectsRegistry).build()
    } flatMap { unit =>
      validator.validate(unit, "RAML 1.0 Dialect")
    } flatMap { report =>
      println(report)
  ...
  test("Custom dialect can be validated (k8)") {
    val validation               = Validation(platform)
    var dialect: Option[Dialect] = None
    val dialectFile              = "file://amf-tests/shared/src/test/resources/vocabularies/k8/dialects/pod.raml"
    val dialectExampleFile       = "file://amf-tests/shared/src/test/resources/vocabularies/k8/examples/pod.raml"
  ...
  test("Custom dialect can be validated (amc2)") {
    val validation               = Validation(platform)
    var dialect: Option[Dialect] = None
    val dialectFile              = "file://amf-tests/shared/src/test/resources/vocabularies/amc2/dialect.raml"
    val dialectExampleFile       = "file://amf-tests/shared/src/test/resources/vocabularies/amc2/example.raml"
  ...
  test("Custom dialect can be validated (amf-eng-demos)") {
    val validation               = Validation(platform)
    var dialect: Option[Dialect] = None
    val dialectFile              = "file://amf-tests/shared/src/test/resources/vocabularies/eng_demos/dialect.raml"
    val dialectExampleFile       = "file://amf-tests/shared/src/test/resources/vocabularies/eng_demos/demo.raml"
  ...
  test("Custom dialect with custom validation can be validated (amf-eng-demos)") {
    ...
    val dialectValidationProfileFile =
      "file://amf-tests/shared/src/test/resources/vocabularies/eng_demos/validation_profile.raml"
  ...
  test("Custom dialect can be validated (evented_apis)") {
    val validation               = Validation(platform)
    var dialect: Option[Dialect] = None
    val dialectFile              = "file://amf-tests/shared/src/test/resources/vocabularies/evented_apis/dialect.raml"
    val dialectExampleFile       = "file://amf-tests/shared/src/test/resources/vocabularies/evented_apis/example/example.raml"
  ...
  test("Custom dialect using lib in dialect") {
    val validator = Validation(platform)
    var dialect: Option[Dialect] = None
    platform.dialectsRegistry.registerDialect("file://amf-tests/shared/src/test/resources/vocabularies/dialect_lib/main_dialect.raml").flatMap { parsedDialect =>
      dialect = Some(parsedDialect)
      AMFCompiler("file://amf-tests/shared/src/test/resources/vocabularies/dialect_lib/example.raml", platform, RamlYamlHint, None, None, platform.dialectsRegistry).build()
    } flatMap { unit =>
