...
/**
  ...
  */
class CharSequenceStream(val data: CharSequence, val startOffset: Int, val endOffset: Int, val sourceName: String = "") extends CharStream {
    ...
    override def lookAhead(i: Int): Int = if (i >= 0 && pointer + i < endOffset) data.charAt(pointer + i) else CharStream.EOF_CHAR
