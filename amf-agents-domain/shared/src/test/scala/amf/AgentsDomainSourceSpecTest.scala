...
class AgentsDomainSourceSpecTest extends AsyncFunSuiteWithPlatformGlobalExecutionContext with Matchers {

  private val basePath = "file://amf-agents-domain/shared/src/test/resources/instances/"

  test("Parsed JSON-LD from AgentsDomain should have AgentsDomain source spec") {
    val client = AgentsDomainConfiguration.AgentsDomain().baseUnitClient()
    for {
      ...
      jsonLdUnit.sourceSpec shouldBe Spec.AGENTS_DOMAIN
    }
  }
