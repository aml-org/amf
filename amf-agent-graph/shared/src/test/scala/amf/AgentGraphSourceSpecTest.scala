...
class AgentGraphSourceSpecTest extends AsyncFunSuiteWithPlatformGlobalExecutionContext with Matchers {

  private val basePath      = "file://amf-agent-graph/shared/src/test/resources/instances/"
  private val renderOptions = RenderOptions().withPrettyPrint.withCompactUris.withEntityEmission
  private val client        = AgentGraphConfiguration.AgentGraph().withRenderOptions(renderOptions).baseUnitClient()

  test("Parsed JSON-LD from AgentGraph should have AgentGraph source spec") {
    for {
      ...
      cycledResult.sourceSpec shouldBe Spec.AGENT_GRAPH
