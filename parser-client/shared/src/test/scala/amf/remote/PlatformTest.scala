...
class PlatformTest extends AsyncFunSuite with ListAssertions with PlatformSecrets {
  ...
  test("File") {
    platform
      .resolve("file://parser-client/shared/src/test/resources/input.yaml") map {
      case Content(content, _, mime) =>
        ...
        content.sourceName should be("parser-client/shared/src/test/resources/input.yaml")
    }
  }
  ...
  test("Path resolution") {
    Future.successful({
      val url = "file://parser-client/shared/src/test/resources/input.yaml"

      platform.resolvePath(url) should be(url)
      platform.resolvePath("file://parser-client/shared/src/test/resources/ignored/../input.yaml") should be(url)
    })
  }
