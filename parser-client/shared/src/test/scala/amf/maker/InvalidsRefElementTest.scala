  ...
  test("Invalid link to Response with ast") {
    Validation(platform)
      .flatMap(v => {
        ...
        build("file://parser-client/shared/src/test/resources/invalids/error-response.oas",
              RamlYamlHint,
              validation = Some(v))
      })
      .map(unit => {
  ...
  test("Invalid link to Trait with ast") {
    Validation(platform)
      .flatMap(v => {
        ...
        build("file://parser-client/shared/src/test/resources/invalids/error-trait.raml",
              RamlYamlHint,
              validation = Some(v))
      })
      .map(unit => {
  ...
  test("Invalid link to resource type with ast") {
    Validation(platform)
      .flatMap(v => {
        ...
        build("file://parser-client/shared/src/test/resources/invalids/error-resource-type.raml",
              RamlYamlHint,
              validation = Some(v))
      })
      .map(unit => {
  ...
  test("Invalid link to security scheme type with ast") {
    Validation(platform)
      .flatMap(v => {
        ...
        build("file://parser-client/shared/src/test/resources/invalids/error-security-scheme.raml",
              RamlYamlHint,
              validation = Some(v))
      })
      .map(unit => {
  ...
  test("Invalid link to named example with ast") {
    Validation(platform)
      .flatMap(v => {
        ...
        build("file://parser-client/shared/src/test/resources/invalids/error-named-example.raml",
              RamlYamlHint,
              validation = Some(v))
      })
      .map(unit => {
