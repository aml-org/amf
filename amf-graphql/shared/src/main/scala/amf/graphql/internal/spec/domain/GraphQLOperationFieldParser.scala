...
case class GraphQLOperationFieldParser(ast: Node)(implicit val ctx: GraphQLBaseWebApiContext)
  ...
  def parse(setterFn: ShapeOperation => Unit): Unit = {
    ...
    inFederation { implicit fCtx =>
      FederationMetadataParser(ast, operation, Seq(FIELD_DIRECTIVE, FIELD_FEDERATION_DIRECTIVE)).parse()
  ...
  private def parseArgument(n: Node): ShapeParameter = {
    ...
    inFederation { implicit fCtx =>
      FederationMetadataParser(n, queryParam, Seq(INPUT_VALUE_DIRECTIVE, INPUT_FIELD_FEDERATION_DIRECTIVE)).parse()
