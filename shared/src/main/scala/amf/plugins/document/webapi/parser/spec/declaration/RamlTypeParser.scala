...
object RamlTypeParser {
  def apply(ast: YMapEntry, adopt: Shape => Shape, isAnnotation: Boolean = false, defaultType: DefaultType = StringDefaultType)(
    ...
    new RamlTypeParser(ast, ast.key, ast.value, adopt, isAnnotation, defaultType)(new WebApiContext(Raml, ctx, RamlSpecAwareContext, RamlSyntax))
}
      ...
      node.tagType match {
        ...
        case _ if node.toOption[YScalar].isDefined =>
          ...
          refTuple match {
            ...
            case (text: String, _) =>
              val shape = UnresolvedShape(text, node).withName(text).withContext(ctx)
              adopt(shape)
              shape
          }
      }
