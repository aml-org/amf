...
class CompatibilityModelAssertionTest extends AsyncFunSuite with FileAssertionTest {
  ...
  test("Test no empty request in RAML to OAS conversion 1") {
    compatibility("raml10/empty-body-1.raml", Raml10YamlHint, Oas30YamlHint) map { output =>
  ...
  test("Test no empty request in RAML to OAS conversion 2") {
    compatibility("raml10/empty-body-2.raml", Raml10YamlHint, Oas30YamlHint) map { output =>
      assert(output.nonEmpty)
      assert(!output.contains("content: {}"))
    }
  }
