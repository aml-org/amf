...
class ValidationTest extends AsyncFunSuite with PlatformSecrets {
  ...
  val payloadsPath     = "file://amf-tests/shared/src/test/resources/payloads/"
  ...
  test("Validation test, custom validation profile") {
    ...
    for {
      ...
      assert(result.targetNode == "file://amf-tests/shared/src/test/resources/validations/data/error1.raml#/web-api")
