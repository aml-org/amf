...
trait DialectWrapper {
  ...
  def prefixes(node: DomainEntity): mutable.Map[String, String] = {
...
case class ValidationSpecification(name: String,
  ...
  def isParserSide(): Boolean = targetClass.nonEmpty && targetClass.head == ValidationSpecification.PARSER_SIDE_VALIDATION
}
