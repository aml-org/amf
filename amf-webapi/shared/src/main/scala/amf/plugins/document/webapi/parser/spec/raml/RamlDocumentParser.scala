          ...
          case YType.Map =>
            e.value.as[YMap].entries.foreach { entry =>
              ctx.declarations += SecuritySchemeParser(entry, scheme => {
                scheme.set(SecuritySchemeModel.Name, AmfScalar(entry.key.as[String], Annotations(entry.key.value)), Annotations(entry.key))
            ...
            e.value
              ...
              .map { entry =>
                ...
                val customProperty = AnnotationTypesParser(entry,
                                                           customProperty => {
                                                             customProperty.set(CustomDomainPropertyModel.Name, AmfScalar(typeName, Annotations(entry.key.value)), Annotations(entry.key))
        ...
        case YType.Map =>
          e.value.as[YMap].entries.foreach { entry =>
            Raml10TypeParser(entry, shape => {
              shape.set(ShapeModel.Name, AmfScalar(entry.key.as[String], Annotations(entry.key.value)), Annotations(entry.key))
