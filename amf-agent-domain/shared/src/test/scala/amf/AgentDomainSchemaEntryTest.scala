...
import amf.agentdomain.internal.plugins.parse.entry.AgentDomainSchemaVersionEntry
...
class AgentDomainSchemaEntryTest extends AsyncFunSuiteWithPlatformGlobalExecutionContext with Matchers {

  private val basePath: String = "amf-agent-domain/shared/src/test/resources/instances/entry/"

  test("AgentDomain with string schemaVersion") {
    val maybeVersion = AgentDomainSchemaVersionEntry.apply(getRoot(basePath + "any-string.json"))
    maybeVersion.nonEmpty shouldBe true
    maybeVersion.get.version shouldBe "0.1.0-beta"
  }

  test("AgentDomain with number schemaVersion") {
    val maybeVersion = AgentDomainSchemaVersionEntry.apply(getRoot(basePath + "any-number.json"))
    maybeVersion.nonEmpty shouldBe true
    maybeVersion.get.version shouldBe "0.1"
  }

  test("AgentDomain without protocol entry") {
    val maybeVersion = AgentDomainSchemaVersionEntry.apply(getRoot(basePath + "none.json"))
