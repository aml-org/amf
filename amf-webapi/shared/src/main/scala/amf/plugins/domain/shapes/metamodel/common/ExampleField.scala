...
/**
  ...
  */
trait ExampleField {
  val Examples = Field(Array(ExampleModel), Http + "example", ModelDoc(ModelVocabularies.Http, "example", "Single example for a particular domain element"))
}

trait ExamplesField {
  val Examples = Field(Array(ExampleModel), Document + "examples", ModelDoc(ModelVocabularies.Http, "examples", "Examples for a particular domain element"))
}
