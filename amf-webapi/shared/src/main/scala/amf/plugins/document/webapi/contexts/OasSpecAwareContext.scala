...
case class Oas2VersionFactory(implicit val ctx: OasWebApiContext) extends OasSpecVersionFactory {
  ...
  override def serversParser(map: YMap, operation: Operation): OasServersParser = Oas3ServersParser(map, operation, OperationModel.Servers)
...
case class Oas3VersionFactory(implicit val ctx: OasWebApiContext) extends OasSpecVersionFactory {
  ...
  override def serversParser(map: YMap, operation: Operation) = Oas3ServersParser(map, operation, OperationModel.Servers)
}
