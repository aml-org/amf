...
//      .withProperties(Seq("http://a.ml/vocabularies/doc#test").asClient)
  ...
  test("AutogeneratedName annotation should exists for anonymous union") {
    ...
    for {
      ...
      val schema = resolutionResult.baseUnit
        ...
        .encodes
        .asInstanceOf[WebApi]
        .endPoints.asSeq.head
        .operations.asSeq.head
        .request
        .payloads.asSeq.head
        .schema
  ...
  test("Test test OAS") {
    val client = OASConfiguration.OAS30().baseUnitClient()
    val path   = "file://amf-cli/shared/src/test/resources/empty-fragment-oas/api.yaml"
    for {
      parsingResult    <- client.parse(path).asFuture
      resolutionResult <- Future.successful(client.transform(parsingResult.baseUnit, PipelineId.Editing))
    } yield {
  ...
  test("Test test RAML") {
    val client = RAMLConfiguration.RAML10().baseUnitClient()
    val path = "file://amf-cli/shared/src/test/resources/empty-fragment-raml/api.raml"
    for {
      parsingResult <- client.parse(path).asFuture
      resolutionResult <- Future.successful(client.transform(parsingResult.baseUnit, PipelineId.Editing))
    } yield {
