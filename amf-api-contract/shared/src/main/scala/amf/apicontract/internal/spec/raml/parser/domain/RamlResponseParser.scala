...
/** */
case class Raml10ResponseParser(entry: YMapEntry, adopt: Response => Unit, parseOptional: Boolean = false)(implicit
...
case class Raml08ResponseParser(entry: YMapEntry, adopt: Response => Unit, parseOptional: Boolean = false)(implicit
    ctx: RamlWebApiContext
) extends RamlResponseParser(entry, adopt, parseOptional) {
...
abstract class RamlResponseParser(entry: YMapEntry, adopt: Response => Unit, parseOptional: Boolean = false)(implicit
    ctx: RamlWebApiContext
  ...
  def parse(): Response = {
    ...
    val response: Response = entry.value.tagType match {
      case YType.Null =>
        val response =
          Response(entry).withName(node).setWithoutId(ResponseModel.StatusCode, node.text(), Annotations.inferred())
        adopt(response)
        response
        ...
        map.key(
          ...
          entry => {
            ...
            entry.value.tagType match {
              case YType.Null =>
                ctx.factory
                  .typeParser(entry, shape => tracking(shape.withName("default"), payload), false, defaultType)
                  .parse()
                  .foreach { schema =>
                    schema.annotations += SynthesizedField()
                    payload.annotations += EmptyPayload()
                    ctx.autoGeneratedAnnotation(schema)
                    payloads += payload.setWithoutId(PayloadModel.Schema, schema, Annotations(entry.value))
                  }
                res.setWithoutId(
              ...
              case YType.Str =>
                ctx.factory
                  ...
                  .foreach(s => {
                    ...
                    payloads += payload.setWithoutId(PayloadModel.Schema, s, Annotations(entry.value))
                  })
                res.setWithoutId(
                  RequestModel.Payloads,
                  AmfArray(payloads, Annotations(entry.value)),
