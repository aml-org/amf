...
class DynamicGraphParser(var nodes: Map[String, AmfElement]) extends GraphParserHelpers {
  ...
  def retrieveType(ast: AMFAST, ctx: GraphContext): Option[(Annotations) => AmfObject] = {
    ts(ast).find({ t =>
      dynamicBuilders.get(ctx.expand(t)).isDefined
    }) match {
        ...
        children.foreach { entry =>
          entry.`type` match {
            case Entry =>
              ...
              if (propertyUri != "@type" && propertyUri != "@id" && propertyUri != DomainElementModel.Sources.value
                    .iri()) {
  ...
  private val dynamicBuilders: mutable.Map[String, (Annotations) => AmfObject] = mutable.Map(
    amf.domain.extensions.ArrayNode.builderType.iri() -> { (a: Annotations) =>
      amf.domain.extensions.ArrayNode(a)
    },
    amf.domain.extensions.ScalarNode.builderType.iri() -> { (a: Annotations) =>
      amf.domain.extensions.ScalarNode(a)
    },
    amf.domain.extensions.ObjectNode.builderType.iri() -> { (a: Annotations) =>
      amf.domain.extensions.ObjectNode(a)
    }
  )
