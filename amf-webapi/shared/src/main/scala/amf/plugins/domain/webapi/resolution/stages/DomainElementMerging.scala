    ...
    other.fields.fields().filter(ignored).foreach {
      case entry @ FieldEntry(field, value) =>
        main.fields.entry(field) match {
          ...
          case Some(existing) if Option(existing.value).isDefined && Option(existing.value.value).isDefined && value.value.isInstanceOf[AnyShape]
            && existing.value.value.isInstanceOf[AnyShape] && existing.value.value.annotations.contains(classOf[Inferred]) =>
  ...
  protected def ensureNotRecursive(shape: Shape, ids: Set[String] = Set()): Shape = {
    try {
      if (ids.contains(shape.id))
        ...
        shape.fields.foreach { case (f: Field, value: Value) =>
