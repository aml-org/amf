...
private[resolution] object ReferencesFixer {
  def apply(s: Shape, context: NormalizationContext): Shape = ReferencesFixer()(context).normalize(s)
}

sealed case class ReferencesFixer()(implicit val context: NormalizationContext) {
