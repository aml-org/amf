...
private[this] object ContentEncodingParser extends EntryParser[ScalarShape] with QuickFieldParserOps {
  override def parse(node: ScalarShape, map: YMap)(implicit ctx: ShapeParserContext): Unit = map.key("contentEncoding", Encoding in node)
}

private[this] object ContentMediaTypeParser extends EntryParser[ScalarShape] with QuickFieldParserOps {
  override def parse(node: ScalarShape, map: YMap)(implicit ctx: ShapeParserContext): Unit = map.key("contentMediaType", MediaType in node)
}
