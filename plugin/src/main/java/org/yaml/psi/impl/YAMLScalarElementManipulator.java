...
import org.yaml.psi.YAMLScalar;
...
public class YAMLScalarElementManipulator extends AbstractElementManipulator<YAMLScalarImpl> {
  ...
  public YAMLScalarImpl handleContentChange(@NotNull YAMLScalarImpl element, @NotNull TextRange range, String newContent)
    ...
    try {
      ...
      final YAMLFile dummyYamlFile = YamlElementGenerator.apply(element.getProject()).createDummyYamlWithText(builder.toString());
      ...
      final PsiElement newElement = element.replace(YamlElementGenerator.apply(element.getProject()).createYamlDoubleQuotedString());
