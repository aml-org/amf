...
case class dialectObject(val entity: DomainEntity=DomainEntity(DialectDefinition),override val parent:Option[TopLevelObject]=None) extends TopLevelObject(entity,parent){
  ...
  def vocabularies():Seq[ExternalObject]= entity.entities(DialectDefinition.vocabularies).map(ExternalObject(_,Some(this)))
  ...
  def external():Seq[ExternalObject]= entity.entities(DialectDefinition.externals).map(ExternalObject(_,Some(this)))
  def withExternal(value:ExternalObject):dialectObject= {entity.add(DialectDefinition.externals.field , value.entity); this}
  def nodeMappings():Seq[NodeDefinitionObject]= entity.entities(DialectDefinition.nodeMappings).map(NodeDefinitionObject(_,Some(this)))
...
case class NodeDefinitionObject(val entity: DomainEntity=DomainEntity(NodeDefinition),override val parent:Option[TopLevelObject]=None) extends TopLevelObject(entity,parent){
  ...
  def mapping():Seq[PropertyMappingObject]= entity.entities(NodeDefinition.mapping).map(PropertyMappingObject(_,Some(this)))
...
case class PropertyMappingObject(val entity: DomainEntity=DomainEntity(PropertyMapping),override val parent:Option[TopLevelObject]=None) extends TopLevelObject(entity,parent){
...
case class DocumentObject(val entity: DomainEntity=DomainEntity(MainNode),override val parent:Option[TopLevelObject]=None) extends TopLevelObject(entity,parent){
  ...
  def module():Option[ModuleDeclarationObject]= entity.entity(MainNode.module).map(ModuleDeclarationObject(_,Some(this)))
  def withModule(value:ModuleDeclarationObject):DocumentObject= {entity.set(MainNode.module.field , value.entity); this}
  def fragments():Option[FragmentsDeclarationObject]= entity.entity(MainNode.fragment).map(FragmentsDeclarationObject(_,Some(this)))
  def withFragments(value:FragmentsDeclarationObject):DocumentObject= {entity.set(MainNode.fragment.field , value.entity); this}
}

case class DocumentContentDeclarationObject(val entity: DomainEntity=DomainEntity(DocumentEncode),override val parent:Option[TopLevelObject]=None) extends TopLevelObject(entity,parent){
  ...
  def resolvedEncodes():Option[NodeDefinitionObject]= resolveReference(DocumentEncode.encodes,
(r,s)=>{r.asInstanceOf[dialectObject].nodeMappings.find(_.entity.id==s)}
 ,e=>NodeDefinitionObject(e,Some(this)))
}

case class DeclarationObject(val entity: DomainEntity=DomainEntity(NodeReference),override val parent:Option[TopLevelObject]=None) extends TopLevelObject(entity,parent){
...
case class ModuleDeclarationObject(val entity: DomainEntity=DomainEntity(ModuleDeclaration),override val parent:Option[TopLevelObject]=None) extends TopLevelObject(entity,parent){
...
case class FragmentsDeclarationObject(val entity: DomainEntity=DomainEntity(FragmentDeclaration),override val parent:Option[TopLevelObject]=None) extends TopLevelObject(entity,parent){
