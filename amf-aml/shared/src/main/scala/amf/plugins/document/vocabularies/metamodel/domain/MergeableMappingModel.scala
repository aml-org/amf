...
trait MergeableMappingModel {
  val MergePolicy       = Field(Str, Namespace.Meta + "mergePolicy", ModelDoc(ModelVocabularies.Meta, "merge policy", "Indication of how to merge this graph node when applying a patch document"))
