...
abstract class BaseAvroSchemaPayloadValidator(
  ...
  protected def loadAvroSchema(text: String): LoadedObj = loadAvro(text)
  ...
  private def getRawAvroSchema(shape: Shape, validationProcessor: ValidationProcessor): Option[CharSequence] = {
    // TODO Aca buscas el valor raw del Avro Schema en el shape. Este valor raw se deberia guardar en una annotation, AvroRaw o algo asi, al inicio del parseo del shape.
    //  Ese raw deberia guardarse para cada Shape, asi como se hace para el AvroType hoy en dia.
    val raw = shape.annotations.find(isInstanceOf[AvroRaw]).map(_.value)
    if (raw.isEmpty) {
      // EMITIRLO?
      // SI POR ALGUNA RAZON NO TIENE LA ANNOTATION, GENERAS UN ERROR (AMFVALIDATIONRESULT) Y SE LO PASAS AL VALIDATION PROCESSOR PARA QUE LO GUARDE
      validationProcessor.keepResults()
    }
    raw
  }
    ...
    payload match {
      ...
      case (Some(obj), resultOption) =>
        ...
        try {
          ...
          case e: Exception => validationProcessor.processException(e, fragmentOption.map(_.encodes))
        }
