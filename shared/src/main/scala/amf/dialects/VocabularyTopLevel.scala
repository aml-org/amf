...
object RAML_1_0_VocabularyTopLevel {
  ...
  case class ClassObject(entity: DomainEntity = DomainEntity(ClassTerm),
    ...
    def resolvedProperties(): List[Option[PropertyObject]] =
      resolveReferences2Options(ClassTerm.properties, (r, s) => {
        r.asInstanceOf[VocabularyObject].propertyTerms.find(_.entity.id == s)
  ...
  case class PropertyObject(entity: DomainEntity = DomainEntity(PropertyTerm),
    ...
    def resolvedExtends(): List[Option[PropertyObject]] =
      resolveReferences2Options(PropertyTerm.`extends`, (r, s) => {
        r.asInstanceOf[VocabularyObject].propertyTerms.find(_.entity.id == s)
      }, e => PropertyObject(e, Some(this)))
  }
