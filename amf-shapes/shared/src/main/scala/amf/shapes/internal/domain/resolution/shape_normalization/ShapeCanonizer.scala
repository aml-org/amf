...
sealed case class ShapeCanonizer()(implicit val context: NormalizationContext) extends ShapeNormalizer {
  ...
  protected def canonicalInheritance(shape: Shape): Shape = {
    if (notDeclaredSimpleInheritance(shape)) {
  ...
  private def isSimpleInheritance(shape: Shape, superTypes: Seq[Shape] = Seq()): Boolean = {
    val singleSuperType = superTypes match {
      case Seq(superType) => Some(superType)
      case _              => None
    }

    shape match {
      case ns: NodeShape => ns.annotations.contains(classOf[DeclaredElement]) && ns.properties.isEmpty
      case _: AnyShape if singleSuperType.isDefined =>
        val superType = singleSuperType.get
        val ignoredFields =
          Seq(
            ShapeModel.Inherits,
            ShapeModel.Name,
  ...
  private def notDeclaredSimpleInheritance(shape: Shape): Boolean = shape match {
