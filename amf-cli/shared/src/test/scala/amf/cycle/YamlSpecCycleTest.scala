...
class YamlSpecCycleTest extends FunSuiteCycleTests {
  ...
  test("Example 5.3 - Mapping key structure indicator") {
    cycle("example-5.3.raml", "example-5.3.raml.raml", Raml10YamlHint, Raml10YamlHint)
  }

  test("Example 6.29 - Node anchors") {
    cycle("example-6.29.raml", "example-6.29.raml.raml", Raml10YamlHint, Raml10YamlHint)
  }
