...
import amf.core.internal.annotations.{LexicalInformation, SourceYPart}
...
def assertAnnotationRange(actual: Any, expectedRange: PositionRange): Assertion = {
  ...
  test("Test properties") {
    ...
    val asserPropertyFunction: (JsonLDObject, String, PositionRange, PositionRange) => Assertion =  assertPropertyAnnotation(instanceLocation)
        parseJsonLSDSchemaAndInstance(schemaLocation, instanceLocation).flatMap(rootObject => {
          ...
          asserPropertyFunction(rootObject, "http://a.ml/vocabularies/core#prop1", PositionRange((2, 2), (2, 20)), PositionRange((2, 11), (2, 20)))
          asserPropertyFunction(rootObject, "http://a.ml/vocabularies/core#prop-array", PositionRange((3, 2), (3, 29)), PositionRange((3, 16), (3, 29)))
          asserPropertyFunction(rootObject, "http://a.ml/vocabularies/core#prop-obj", PositionRange((4, 2), (6, 3)), PositionRange((4, 14), (6, 3)))
          ...
          asserPropertyFunction(propertyObject.value.asInstanceOf[JsonLDObject], "http://a.ml/vocabularies/core#key", PositionRange((5, 4), (5, 12)), PositionRange((5, 11), (5, 12)))
  ...
  def assertPropertyAnnotation(location: String)(obj: JsonLDObject, uri: String, propertyRange: PositionRange, valueRange: PositionRange): Assertion = {
  ...
  def parseJsonLSDSchemaAndInstance(schemaPath: String, instancePath: String): Future[JsonLDObject] = {
    client.parseJsonLDSchema(schemaPath).map(_.jsonDocument).flatMap(schema => {
      client.parseJsonLDInstance(instancePath, schema).map(result => {
