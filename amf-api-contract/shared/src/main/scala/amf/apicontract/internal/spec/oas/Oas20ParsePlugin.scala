...
import amf.apicontract.internal.spec.oas.parser.{Oas2DocumentParser, OasFragmentParser}
...
object Oas20ParsePlugin extends OasParsePlugin {
  ...
  override protected def parseSpecificVersion(root: Root)(implicit ctx: OasWebApiContext): BaseUnit =
    OasHeader(root) match {
      ...
      case Some(Oas20Extension) => parser.Oas2DocumentParser(root).parseExtension()
      case Some(Oas20Header)    => parser.Oas2DocumentParser(root).parseDocument()
