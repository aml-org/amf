...
case class Raml10ParameterParser(entry: YMapEntry,
  ...
  override def parse(): Parameter = {
    ...
    if (p.fields.entry(ParameterModel.Required).isEmpty) {
      ...
      p.set(ParameterModel.Required, AmfScalar(!optionalParam), requiredAnnot, Annotations.synthesized())
      ...
      p.set(ParameterModel.Name, AmfScalar(paramName, name.text().annotations), Annotations.inferred())
        .set(ParameterModel.ParameterName, paramName, Annotations.inferred())
    }
...
case class Raml08ParameterParser(entry: YMapEntry,
  ...
  def parse(): Parameter = {
    ...
    parameter.withSynthetizedBinding(binding)
    adopted(parameter)
