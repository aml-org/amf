...
import amf.shapes.internal.annotations.{ExternalReferenceUrl, InlinedFrom}
...
case class OasResponseParser(map: YMap, adopted: Response => Unit)(implicit ctx: OasWebApiContext)
  ...
  private def defaultPayload(entries: YMap): Option[Payload] = {
    val payload = Payload(Annotations.virtual()).add(DefaultPayload())

    def scalarRefIn(node: YNode): Option[YScalar] =
      node
        .to[YMap]
        .toOption
        .flatMap(_.key("$ref"))
        .map(_.value)
        .filter(_.tagType == YType.Str)
        .flatMap(_.asScalar)
    ...
    entries.key(
      "schema",
      entry => {
        val parsedShape = OasTypeParser(entry, shape => shape.withName("default")).parse().map { s =>
          ctx.autoGeneratedAnnotation(s)
          tracking(s, payload)
        }

        val payloadShape = parsedShape
          .map {
            case s if s.isLink || s.isUnresolved => s
            case s =>
              scalarRefIn(entry.value).foreach(s.annotations += InlinedFrom(_))
              s
          }
          .getOrElse(AnyShape(entry.value))

        payload.setWithoutId(PayloadModel.Schema, payloadShape, Annotations(entry))
      }
    )
