...
class ValidationWithProfileTest extends ValidationReportGenTest {
  ...
  override val reportsPath: String = "parser-client/shared/src/test/resources/validations/reports/with-profiles/"

  private def validate(api: String, profile: String, profileFile: Option[String]) =
    super.validate(api, None, profile, profileFile)
  ...
  test("Banking example validation") {
    validate("banking/api.raml", Some("baking-api.report"), "Banking", Some("banking/profile.raml"))
  }

  ignore("Example JS library validations") {
    validate("libraries/api.raml", Some("libraries-profile.report"), "Test", Some("libraries/profile.raml"))
