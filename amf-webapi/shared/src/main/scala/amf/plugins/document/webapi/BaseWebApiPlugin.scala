...
import amf.plugins.document.webapi.validation.remod.{
  ModelValidatePlugin,
  ParserValidatePlugin,
  PayloadValidatePlugin,
...
trait BaseWebApiPlugin extends AMFDocumentPlugin with AMFValidationPlugin with WebApiValidations with PlatformSecrets {
  ...
  override def validationRequest(
    ...
    validate(baseUnit, profile, validations, env, resolved)
  }
  ...
  private def validate(baseUnit: BaseUnit,
                       ...
                       resolved: Boolean)(implicit executionContext: ExecutionContext): Future[AMFValidationReport] = {
    // Before validating we need to resolve to get all the model information
    val plugins = Seq(ParserValidatePlugin, ModelValidatePlugin, PayloadValidatePlugin)
    RemodValidationRunnerFactory
