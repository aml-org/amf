      ...
      queryEntry => {
        Raml10TypeParser(queryEntry, shape => shape.adopted(request.getOrCreate.id))(
          ...
          .map(q => {
            ...
            finalRequest.set(RequestModel.QueryString, tracking(q, finalRequest), Annotations(queryEntry))
          })
      }
    )
  }
            ...
            ctx.factory
              ...
              .foreach { schema =>
                ...
                payloads += payload
                  ...
                  .set(PayloadModel.Schema, tracking(schema, payload), Annotations(entry.value))
              }
