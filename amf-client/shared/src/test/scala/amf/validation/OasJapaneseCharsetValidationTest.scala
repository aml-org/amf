...
class OasJapaneseCharsetValidationTest extends MultiPlatformReportGenTest {
  ...
  test("Documentation server params") {
    validate("documentation-server-params.json", profile = Oas30Profile)
  }

  test("Documentation path and response description") {
    validate("documentation-path-and-response-description.json", profile = Oas30Profile)
  }

  test("Documentation path item description") {
    validate("documentation-path-item-description.json", profile = Oas30Profile)
  }

  test("Documentation security schemes") {
    validate("documentation-security-schemes.json", profile = Oas30Profile)
  }

  test("Facet string pattern valid") {
    validate("facet-string-pattern-valid.yaml", profile = Oas30Profile)
  }

  test("Facet string pattern invalid") {
    validate("facet-string-pattern-invalid.yaml", Some("facet-string-pattern-invalid.report"), profile = Oas30Profile)
  }

  test("Facet string length valid") {
    validate("facet-string-length-valid.yaml", profile = Oas30Profile)
  }

  test("Extensions") {
    validate("extensions.yaml", profile = Oas30Profile)
  }
  ...
  test("JSON Schema include") {
    validate("json-schema-include.yaml", profile = Oas30Profile)
  }

  test("Documentation info") {
    validate("documentation-info.json", profile = Oas30Profile)
  }
  ...
  test("Non ASCII header names OAS 3.0") {
    validate("non-ascii-headers-oas3.json", Some("non-ascii-headers-oas3.report"), profile = Oas30Profile)
  }
}
