...
import amf.plugins.document.webapi.parser.spec.{declaration, toRaml}
...
case class AsyncSchemaEmitter(key: String,
  ...
  private def emitAsRaml(b: EntryBuilder): Unit = {
    val emitters =
      declaration.emitters.Raml10TypeEmitter(shape, ordering, references = references)(toRaml(spec)).entries()
    b.entry(
      key,
