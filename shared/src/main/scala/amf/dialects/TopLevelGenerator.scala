...
class TopLevelGenerator(d:Dialect) {
  ...
  kw.split("\\s*,\\s*").foreach { v => keywords.add(v) }
  ...
  private def generateNodeRangeIfNeeded(p: DialectPropertyMapping): Unit = {
    ...
    p.referenceTarget.foreach { node => appendNodeIfNeeded(node) }
  }
  ...
  def generateNode(d: DialectNode): String = {
    ...
    sb.append(s"  case class ${d.shortName}Object(entity: DomainEntity = DomainEntity(${nameOfType(d)}), override val parent:Option[TopLevelObject] = None) extends TopLevelObject(entity, parent){\n")
