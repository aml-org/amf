...
class LanguageServerTest extends AsyncFunSuite with CompilerTestBuilder {
  ...
  private val file = "file://parser-client/shared/src/test/resources/ls/resource-type-trait.raml"
  ...
  test("Parse variable in query param") {
    val file = "file://parser-client/shared/src/test/resources/ls/trait_error1.raml"
    build(file, RamlYamlHint)
      .map(_.asInstanceOf[Document])
      .map { model =>
        model.declares
  ...
  test("Parse variable in nested type") {
    val file = "file://parser-client/shared/src/test/resources/ls/trait_error2.raml"
    build(file, RamlYamlHint)
      .map(_.asInstanceOf[Document])
      .map { model =>
        model.declares
  ...
  test("Error in trait 3") {
    val file = "file://parser-client/shared/src/test/resources/ls/trait_error3.raml"
    build(file, RamlYamlHint)
      .map(_.asInstanceOf[Document])
      .map { model =>
