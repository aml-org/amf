...
class SourceNodeAnnotationTest extends AsyncFunSuite with CompilerTestBuilder with Matchers {
  ...
  test("test full raml") {
    build("file://parser-client/shared/src/test/resources/upanddown/full-example.raml", RamlYamlHint) map {
      checkAnnotation
    }
  }

  test("test full oas") {
    build("file://parser-client/shared/src/test/resources/upanddown/full-example.json", OasJsonHint) map {
      checkAnnotation
    }
  }

  test("test full raml with type xml tag") {
    build("file://parser-client/shared/src/test/resources/org/raml/api/v10/xmlbodyinlinewithfacet/input.raml",
  ...
  test("test node and entry annotation in oas parameters") {
    for {
      unit <- build("file://parser-client/shared/src/test/resources/nodes-annotations-examples/oas-parameters.yaml",
                    OasYamlHint)
    } yield {
  ...
  test("test node and entry annotation in oas responses") {
    for {
      unit <- build("file://parser-client/shared/src/test/resources/nodes-annotations-examples/oas-responses.yaml",
                    OasYamlHint)
    } yield {
      val document = unit.asInstanceOf[Document]
  ...
  test("test node and entry annotation in oas definitions and schemes") {
    for {
      unit <- build("file://parser-client/shared/src/test/resources/nodes-annotations-examples/oas-schemes.yaml",
                    OasYamlHint)
    } yield {
      val document = unit.asInstanceOf[Document]
  ...
  test("test node and entry annotation in oas xml shape property") {
    for {
      unit <- build("file://parser-client/shared/src/test/resources/nodes-annotations-examples/oas-xmlSerializer.yaml",
                    OasYamlHint)
    } yield {
      val document = unit.asInstanceOf[Document]
