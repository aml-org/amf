...
case class PropertyMapping(fields: Fields, annotations: Annotations) extends DomainElement {
  ...
  def minimum(): Option[Float]                    = Option(fields(Minimum))
  def withMinimum(min: Float)                     = set(Minimum, min)
  def maximum(): Option[Float]                    = Option(fields(Maximum))
  def withMaximum(max: Float)                     = set(Maximum, max)
