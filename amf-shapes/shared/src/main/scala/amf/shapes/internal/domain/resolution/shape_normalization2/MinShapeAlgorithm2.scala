...
private[resolution] class MinShapeAlgorithm2()(implicit val context: NormalizationContext2)
    extends RestrictionComputation2 {
...
//            .adopted(baseNode.id)
  ...
  object UnionErrorHandler extends AMFErrorHandler {
    ...
    def wrapContext(ctx: NormalizationContext2): NormalizationContext2 = {
      new NormalizationContext2(
        this,
        ctx.keepEditingInfo,
        ctx.profile,
  ...
  protected def computeMinUnion(baseUnion: UnionShape, superUnion: UnionShape): Shape = {
    ...
    val unionContext: NormalizationContext2 = UnionErrorHandler.wrapContext(context)
    val newUnionItems =
